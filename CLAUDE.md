...
- ✅ **Produk jadi multi-bentuk (2–3 Sep 2026)**: karton multi-satuan PACK/SLOP/BAL (0029, satu unit per karton, isi dari pack HLP ATAU output stage — sisa = Σout(stage) − Σin(stage berikutnya) − dialokasikan), finished goods receiving per (shift, unit), produk jadi target per batch diputuskan di HLP (0030: PACK/PACK_WRAP/SLOP/BAL + rantai wajib + ubah ber-alasan), penanda TSG makloon end-to-end + pemesan + produk pesanan (0031), finalisasi SJ supplier di gudang inbound, surat jalan PDF sadar satuan
- ⚠️ **Sisa go-live produksi ada di TODO.md seksi "🎯 Sisa — Menuju go-live produksi"** (re-login user, FCM natural, checklist manual)
- ⚠️ Migrasi manual sampai **0031** (auto-apply entrypoint); dev DB di container `mes_dev_postgres` (port host 5433)
...
9. **Migrasi manual di luar journal drizzle** — dulu wajib apply manual via psql. **Sekarang otomatis**: entrypoint container panggil `scripts/apply-manual-migrations.mjs` setelah `drizzle-kit migrate`, jalankan semua `.sql` di `src/db/migrations/` yang tidak ada di `_journal.json`. Syarat: file wajib idempotent (`IF NOT EXISTS`, `OR REPLACE`, **`DROP POLICY IF EXISTS` sebelum `CREATE POLICY`** — 0024 polos sempat akan mematahkan deploy berikutnya karena policy duplicate) karena di-run setiap deploy. ⚠️ **`ALTER TYPE ... ADD VALUE` TIDAK boleh di file multi-statement** — script mengeksekusi tiap file dalam satu batch (implicit transaction) → error `check_safe_enum_use`. Taruh di file terpisah berisi SATU statement (lihat migrasi 0016 vs 0017). ⚠️ **Tabel `batch` TIDAK punya policy UPDATE di 0000** — update batch (mis. stage) silent no-op tanpa error kalau policy UPDATE tidak ditambahkan.
10. **RLS aktif via role `mes_app`** (migrasi 0008, non-superuser). `DATABASE_URL` = runtime (mes_app), `DATABASE_URL_ADMIN` = superuser untuk drizzle-kit & seed. Jangan ganti DATABASE_URL ke role superuser/owner — RLS mati lagi. Seed pakai `pnpm db:seed*` (wrapper run-seed*.ts, otomatis pakai URL admin). **Seed pada DB yang sudah ter-seed tetap sync role→permission secara additive** (drift fix 1 Sep 2026 — grant permission baru kini sampai ke prod tanpa migrasi; grant manual TIDAK dihapus). **Sejak 3 Sep 2026**: DB ter-seed juga otomatis **INSERT permission code baru** ke tabel permission (sebelumnya hanya grant — permission baru tidak pernah masuk ke DB ter-seed). Script tsx TIDAK memuat `.env` — `set -a; source .env` dulu kalau dipanggil langsung. ⚠️ Kalau tes RLS lewat script manual, format setting-nya array: `set_config('app.current_plant_ids', '{uuid1,uuid2}', false)`
...
15. **Rantai produk jadi (0029–0031)** — karton multi-satuan: satu karton satu `unit` (PACK/SLOP/BAL); isi dari `hlp_pack` (sourceType HLP_PACK) atau output stage (sourceType STAGE, batchId+stage); sisa stage = Σout(stage) − Σin(stage berikutnya) − dialokasikan ke karton. `batch.target_unit` menentukan rantai wajib (PACK=[] · PACK_WRAP=[WR] · SLOP=[WR,SLOP] · BAL=[WR,SLOP,BAL]) — validasi `STAGE_NOT_IN_TARGET`/`STAGE_SEQUENCE_REQUIRED` hanya untuk batch INTERNAL (makloon EXTERNAL tetap model entry/exit stage). FG receiving per (shift, unit). Makloon: `is_makloon`+`makloon_customer`+`makloon_target` di receiving → inventory → batch (timbang) → lineage.
16. **Endpoint finalisasi SJ** — `POST /tsg-receiving/from-sj` (kontrak mobile, permission `tsg.receiving.create`) memanggil `receiveFromSupplierSj`; jangan buat endpoint duplikat. Field makloon opsional sudah ada di sana.
17. **Backward-compat add-pack** — `POST /cartons/:id/add-pack` menerima BOTH body baru (`sourceType`) DAN body legacy (`{hlpPackId, packQty}` → dinormalisasi HLP_PACK). Jangan hapus cabang legacy — kontrak klien lama.
18. **Generalisasi enum satuan** — `carton.unit`/`finished_goods_receiving.unit`/`batch.target_unit` pakai TEXT + CHECK, BUKAN pgEnum (hindari jebakan `ALTER TYPE ADD VALUE` di migrasi multi-statement — gotcha #9).
...
- ⚠️ **Server basi di 3100**: `reuseExistingServer: !CI` berarti kalau ada proses `next start` LAMA di 3100 (build lama), Playwright me-reuse-nya TANPA rebuild — route/UI baru kena 404 atau spec lama. Sebelum run yang butuh build baru: `lsof -iTCP:3100 -sTCP:LISTEN` lalu `kill` prosesnya.
- **Chain antar spec**: `workers: 1`, state lewat `.e2e/e2e-state.json` (gitignored; dihapus reset script). `shiftId`+`batchCode` dari 02 (pivot), `packsLolos` dari 04, `cartonCode` dari 05, `slopCartonCode` dari 05b. Spec downstream `skip` kalau state prasyarat tidak ada — selalu jalankan full suite untuk determinisme. ⚠️ `readState()` adalah snapshot awal test — kode state yang ditulis MID-TEST harus disimpan di variabel lokal (bukan `state.x`). ⚠️ Auto-accept dialog browser (`confirm()`) hanya aktif lewat fixture `login` — spec yang tidak memakai fixture itu akan stuck di `confirm()`.
