// POST /tsg-receiving/:id/reject — Tolak receiving manual dengan catatan
// Request tim mobile: approver (PM/supervisor) bisa menolak receiving PENDING
// + catatan alasan. Status REJECTED tidak membuat inventory.
import { NextResponse } from "next/server";
import { z } from "zod";
import { withAuth, type AuthContext } from "@/lib/auth/middleware";
import { rejectReceiving } from "@/lib/services/wms-inbound.service";
...
export const POST = withAuth(
  async (request: Request, ctx: AuthContext, { params }: { params: Promise<{ id: string }> }) => {
    try {
      ...
      const result = await rejectReceiving(receivingId, plantId, ctx.user.userId, parsed.data.reason);
