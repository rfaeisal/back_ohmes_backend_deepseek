// POST /shifts/:id/maintenance — Log maintenance/sparepart event
...
export const POST = withAuth(
  async (request: Request, ctx: AuthContext, { params }: { params: Promise<{ id: string }> }) => {
    try {
      ...
      const result = await logMaintenance({ shiftReportId, plantId, ...parsed.data, loggedBy: ctx.user.userId });
