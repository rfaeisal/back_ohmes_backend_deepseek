// POST /api/v1/shifts/:id/boxes — Buka boks baru dari inventory
import { NextResponse } from "next/server";
import { z } from "zod";
import { withAuth, type AuthContext } from "@/lib/auth/middleware";
import { openBox, ServiceError as BoxServiceError } from "@/lib/services/box.service";
import { ServiceError } from "@/lib/services/shift.service";

const openBoxSchema = z.object({
  inventoryBoxId: z.string().uuid().optional(),
});
...
export const POST = withAuth(
  async (request: Request, ctx: AuthContext, { params }: { params: Promise<{ id: string }> }) => {
    try {
      ...
      const parsed = openBoxSchema.safeParse(body);
      ...
      const box = await openBox({
        shiftReportId,
        plantId,
        inventoryBoxId: parsed.data.inventoryBoxId,
      });
      ...
      if (err instanceof BoxServiceError || err instanceof ServiceError) {
        const status = err.code === "TSG_BOX_NOT_AVAILABLE" ? 400 : 409;
        return NextResponse.json(
          { error: { code: err.code, message: err.message, details: (err as ServiceError).details }, requestId: ctx.requestId },
          { status }
        );
      }
      throw err;
