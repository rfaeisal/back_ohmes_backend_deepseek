...
import { getAreaKpi, getAreaKpiWeek } from "@/lib/services/area-dashboard.service";
...
export const GET = withAuth(
  async (request: Request, _ctx: AuthContext, { params }: { params: Promise<{ regionId: string }> }) => {
    ...
    const mode = url.searchParams.get("mode") ?? "day"; // day | week
    const weekStart = url.searchParams.get("weekStart") ?? undefined;

    const kpi = mode === "week" && weekStart
      ? await getAreaKpiWeek(regionId, weekStart)
      : await getAreaKpi(regionId, date);
