// POST /api/v1/hlp/shifts/:id/members — tambah anggota sesi (ganti kru tanpa tutup sesi)
import { NextResponse } from "next/server";
import { z } from "zod";
import { withAuth, type AuthContext } from "@/lib/auth/middleware";
import { attachHlpShiftMember } from "@/lib/services/hlp-session.service";
import { ServiceError } from "@/lib/services/shift.service";

const schema = z.object({
  userId: z.string().uuid(),
  shiftRoleId: z.string().uuid().optional(), // roster hanya default — bebas pilih
});

export const POST = withAuth(
  async (request: Request, ctx: AuthContext, { params }: { params: Promise<{ id: string }> }) => {
    try {
      const { id: shiftId } = await params;
      const body = await request.json();
      const parsed = schema.safeParse(body);
      if (!parsed.success) {
        return NextResponse.json(
          { error: { code: "VALIDATION_ERROR", message: "Input tidak valid.", details: parsed.error.flatten() }, requestId: ctx.requestId },
          { status: 400 }
        );
      }

      const member = await attachHlpShiftMember(
        shiftId,
        parsed.data.userId,
        parsed.data.shiftRoleId,
        ctx.user.userId
      );
      return NextResponse.json(member, { status: 201 });
    } catch (err) {
      if (err instanceof ServiceError) {
        return NextResponse.json(
          { error: { code: err.code, message: err.message }, requestId: ctx.requestId },
          { status: 409 }
        );
      }
      throw err;
    }
  },
  { requiredPermission: "hlp.pack" }
);
