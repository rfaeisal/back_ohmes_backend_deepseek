...
const patchSchema = z.object({ name: z.string().optional(), address: z.string().optional(), code: z.string().optional() });
...
export const PATCH = withAuth(async (request: Request, _ctx: any, { params }: { params: Promise<{ id: string }> }) => {
  ...
  await db.update(plant).set(parsed.data).where(eq(plant.id, id));
