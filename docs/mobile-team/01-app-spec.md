# 01 · Spesifikasi Teknis Aplikasi Mobile (Flutter)

Dokumen operasional untuk **implementasi aplikasi mobile Flutter**. Berisi auth flow spesifik mobile, aturan single-session, local queue, deep link, push notification, dan testing checklist.

**Ditujukan**: mobile developer Flutter yang mengimplementasi app dari scratch.
**Related**: [`02-api-contract.md`](./02-api-contract.md), [`03-qr-strategy.md`](./03-qr-strategy.md), [`04-glossary.md`](./04-glossary.md), [`05-rbac-mobile.md`](./05-rbac-mobile.md).
...
Sudah dibahas di [`03-qr-strategy.md`](./03-qr-strategy.md) §6.3. Ringkasnya:
...
Sudah dibahas di [`03-qr-strategy.md`](./03-qr-strategy.md) §6.2. Ringkasnya:
...
## 13. Referensi (dalam paket ini)

- [`02-api-contract.md`](./02-api-contract.md) §2 — auth endpoints (login single-session, refresh, switch-scope, me/sessions).
- [`02-api-contract.md`](./02-api-contract.md) §4 — operational endpoints (shift lifecycle, boxes, events, waste, handoff).
- [`02-api-contract.md`](./02-api-contract.md) §4A — WMS Inbound endpoints (receiving, inventory).
- [`02-api-contract.md`](./02-api-contract.md) §6 — QR resolve endpoint.
- [`03-qr-strategy.md`](./03-qr-strategy.md) — QR lifecycle + Flutter deep link + local queue detail.
- [`04-glossary.md`](./04-glossary.md) — istilah domain (TSG, HLP, Kecer, dsb).
- [`05-rbac-mobile.md`](./05-rbac-mobile.md) — permission per role untuk endpoint yang mobile pakai.

**Detail internal** (tidak dalam paket ini — tanya PM/backend lead):
- Skema data backend (Drizzle) — hanya perlu tahu bentuk request/response API di §2.
- Multi-tenant RLS mekanik — dari perspektif mobile, transparan (server handle).
- Materialized view rollup HQ — bukan urusan mobile.
