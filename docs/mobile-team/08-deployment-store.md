# 08 · Deployment ke Play Store & App Store

Panduan submit + release aplikasi mobile Flutter ke Google Play Store (Android) dan Apple App Store (iOS).
...
**Rekomendasi utama** untuk operator lantai produksi:
- Provisioned device (dari IT perusahaan) — bukan personal device operator.
...
- Backend: header `X-Client-Version: 1.2.3`.
- Analytics: user property `app_version`.

### 2.1. Min Supported Version

Backend enforce min version via `X-Client-Version`:
- Client < min → 426 `UPGRADE_REQUIRED`.
- Force user update sebelum bisa pakai.

Update min version di backend saat:
- Breaking API change.
- Security patch mandatory.

Minimum update lag: 4 minggu after release baru (kasih waktu user update).
...
- Package name: `com.hummergroup.mes` (unique, tidak bisa diganti).
...
- Bundle ID: `com.hummergroup.mes` (harus match iOS project).
...
- [ ] Verify signing.
- [ ] Test upgrade in-place (bukan clean install) — critical: session tidak boleh hilang.
...
### 6.3. Backend Mitigasi
- Kalau bug mobile parah, backend bisa:
  - Bump `X-Client-Version` min supported → force update user ke version lama yang stable.
  - Enable feature flag off untuk fitur bermasalah.
...
- WhatsApp group internal untuk operator (Fase 1 pilot).
...
- [`01-app-spec.md`](./01-app-spec.md) §8 — versioning + force update.
