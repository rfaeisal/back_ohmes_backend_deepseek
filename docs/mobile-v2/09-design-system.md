# 09 · Design System — Mobile v2

> **Sumber**: disalin dari `../mobile-team/06-design-system.md` (pack v1.3.0) dan disesuaikan untuk mobile v2 (snapshot 2026-08-22, backend live production). Isi lama yang masih relevan **dipertahankan utuh**; ditambahkan **§15 Komponen khusus fitur baru** (scan label SJ, pool counter, badge status SJ, kartu stok TSG) sesuai enum & endpoint aktual di branch `main`. Brand color masih placeholder Hummer — lihat §2.1.
...
**Kalau tidak ada Figma**: build UI dari spec text di sini + spec layar flow (lihat [`06-flow-supplier-sj.md`](./06-flow-supplier-sj.md) untuk flow SJ — satu-satunya flow dengan screen blueprint). Iterate dengan user feedback.

---

## 15. Komponen Khusus Fitur Baru (Mobile v2)

Komponen tambahan untuk flow SJ supplier, receiving, dan monitoring. Semua mengikuti sistem di atas (spacing 4dp grid, tap target ≥48dp, tombol primary 88dp, card radius 8dp). Enum yang dipakai diverifikasi dari `src/db/schema` branch `main`:

- **`tsgTypeEnum`** (`src/db/schema/wms-inbound.ts`): `REGULER | MILD | PUTIHAN`.
- **`supplierSjStatusEnum`** (`src/db/schema/supplier-sj.ts`): `DRAFT | SHIPPED | RECEIVED`.
- **`supplierSjLabelStatusEnum`**: `AVAILABLE | ASSIGNED | VOID`.
- Format boxCode label QR: `TSG-YYYYMMDD-NNN`.

### 15.1. Scan Label SJ (AREA_SJ_OFFICER)

Alur satu langkah di gudang supplier: scan QR → kartu label → pilih jenis (kalau label pool) → input berat → simpan.

Backend: `GET /supplier-sj/labels/:boxCode` (resolve hasil scan) lalu `POST /supplier-sj/:id/boxes/weigh` dengan body `{boxCode, tsgType?, supplierWeightKg}` — `tsgType` **wajib** saat label masih pool (belum punya jenis).

**Layar kamera**:
- Kamera full-screen; frame scan **280x280dp** di tengah (border 2px `primary`, corner accent, radius 12dp).
- Garis scan animasi horizontal (durasi 1200ms loop).
- Torch toggle: icon button 48x48 di kanan atas.
- Input manual fallback: text field di bawah frame (ketik boxCode saat QR rusak/terlipat).
- Saat resolve: skeleton kartu + spinner inline (jangan full-screen — petugas bisa langsung scan label berikutnya).
- Error resolve (404 `LABEL_NOT_FOUND`, label milik petugas lain, atau VOID): toast error + shake (§11.4), frame scan tetap aktif.

**Hasil scan — kartu label** (Card §6.4, dense):
- Header: `boxCode` (`TSG-YYYYMMDD-NNN`) — `numericMedium` + tabular figures, monospace.
- Badge jenis TSG (REGULER/MILD/PUTIHAN — §15.3). Kosong jika label pool belum discan.
- Baris data: **Berat (kg)** — `numericLarge`; **Status label** — chip semantic (AVAILABLE `success`, ASSIGNED `info`, VOID `disabled`).
- Footer (kalau label sudah ASSIGNED): nama supplier + SJ number.

**Pilihan jenis besar** — hanya muncul saat label pool tanpa `tsgType`:
- 3 tombol stacked, full-width, **min-height 72dp** (untuk sarung tangan), radius 8dp:
  - **REGULER** — bg `primary` (vermilion), teks putih.
  - **MILD** — bg `info` (`0xFF3B82F6`), teks putih.
  - **PUTIHAN** — bg `secondary` (`0xFF1F2937`), teks putih.
- Teks `labelLarge` uppercase + ikon 24x24.
- Setelah pilih jenis → keyboard numerik besar untuk input berat (field height 56dp, `tabular-nums`) → tombol **"Simpan & Timbang"** 88dp primary (`primaryAction` §5.1).
- Label sudah ASSIGNED ke SJ lain: tampilkan kartu read-only + tombol "Scan Berikutnya" (tidak ada opsi timbang ulang).

### 15.2. Pool Counter (sisa label pool)

Sumber data: `GET /supplier-sj/pool` → `{available, assigned, voided, byPrintDate}`.

**Tampilan** (dashboard area office / sebelum buat SJ):
- Stat tile kompak: angka **available** — `numericLarge` 32 + label "Label tersedia" (`labelMedium`, `muted`).
- Chip kecil: AVAILABLE (`success`), ASSIGNED (`info`), VOID (`disabled`).
- **Low-stock warning**: available < 50 → tile bg amber 15%, teks "Sisa {n} label — cetak pool baru" (warna `warning`).
- Tap tile → bottom sheet: breakdown per tanggal cetak (`byPrintDate`, list tanggal + jumlah).
- Refresh: pull-to-refresh + auto-refresh 60 detik saat layar aktif. Saat offline: tampilkan nilai cache terakhir + indikator stale (badge "Data lama" `warning`).

### 15.3. Badge Status SJ

Status enum backend `supplier_sj_status`: `DRAFT → SHIPPED → RECEIVED`.

| Status | Warna | Makna |
|---|---|---|
| `DRAFT` | `warning` amber (`0xFFF59E0B`) | SJ dibuat, belum semua boks tertimbang |
| `SHIPPED` | `info` blue (`0xFF3B82F6`) | Semua boks tertimbang, truk berangkat ke pabrik |
| `RECEIVED` | `success` green (`0xFF10B981`) | Pabrik verifikasi & receiving selesai |

Spec: pill rounded (radius 999), padding 8x4, `labelMedium` uppercase + dot 8x8 senada. Dipakai di: daftar SJ (kolom status), header detail SJ, kartu hasil scan (§15.1).

**Badge jenis TSG** memakai pola pill yang sama **tanpa dot**:
- `REGULER` = `primary` vermilion
- `MILD` = `info` blue
- `PUTIHAN` = `secondary` deep slate

### 15.4. Kartu Stok TSG (layar monitoring)

Untuk dashboard monitoring (PLANT_MANAGER / AREA_COORDINATOR / AREA_QA): stok TSG per plant per jenis.

Card (radius 8dp, padding 16, §6.4):
- **Header**: kode plant (`labelMedium`, `secondary`) + badge jenis TSG (§15.3).
- **Body**: berat total — `numericLarge` + "kg" (`bodyMedium`, `muted`).
- **Footer**: umur stok — "Umur {n} hari" (`bodySmall`, `muted`) + **aging color**:
  - < 7 hari: normal (`success`).
  - 7–30 hari: perhatian (`warning`).
  - \> 30 hari: stale (`error`) + label "Stale".
- **Dense variant** (list view): tinggi 64dp, berat di kanan (`numericMedium`), tanpa footer.

Loading: skeleton card. Empty state: ikon 64 + "Belum ada stok TSG" (§6.7).

---

## 16. Referensi

- [`00-brief.md`](./00-brief.md) — konteks user & role mobile v2.
- [`03-api-contract.md`](./03-api-contract.md) — endpoint yang dipakai komponen (§15: supplier-sj, pool, tsg-receiving/from-sj).
- [`06-flow-supplier-sj.md`](./06-flow-supplier-sj.md) — spec layar flow SJ (screen blueprint).
- [`08-flow-monitoring.md`](./08-flow-monitoring.md) — alur dashboard & stok TSG.
