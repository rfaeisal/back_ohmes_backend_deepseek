# 07 · QR Code Strategy & Mobile Guidance

QR code adalah **kunci UX Fase 3** — mengganti input manual boks/mesin/batch dengan scan. Panduan ini membahas format URI, lifecycle 4 jenis QR, tabel `QRRegistry`, dan guidance untuk Flutter app.

> **Timing**: Schema `qr_registry` disiapkan di **Fase 0** supaya migrasi ke Fase 3 tidak butuh perubahan skema. Endpoint `/qr/*` diaktifkan di Fase 3.
...
## 4. Skema `qr_registry` (dari [`04-data-model.md`](./04-data-model.md) §8)
...
## 5. Endpoint QR (dari [`06-api-spec.md`](./06-api-spec.md) §6)
...
| Fase | Yang dilakukan |
|---|---|
| **0** | Skema `qr_registry`, `qr_scan_log` sudah ada. Enum `qr_type` didefinisikan. Tidak ada endpoint aktif. |
| **1** | Belum ada QR di flow — operator input manual. |
| **3** | Print system untuk QR mesin & TSG box. Flutter app dengan scanner + local queue. Endpoint `/qr/*` aktif. |
| **4** | Batch & pack QR untuk traceability lengkap. Analitik scan pattern (mis. mesin yang jarang di-scan = alat rusak?). |

---

## 9. Referensi
- [`04-data-model.md`](./04-data-model.md) §8 — skema `qr_registry`.
- [`06-api-spec.md`](./06-api-spec.md) §6 — endpoint QR.
- [`08-roadmap.md`](./08-roadmap.md) Fase 3 — timeline mobile.
